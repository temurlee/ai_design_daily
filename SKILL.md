...
description: Generate a Chinese AI design daily report from real X/Twitter posts in the last 24 hours. Use when user asks for AI日报/设计日报/AI自媒体日报. Output: 今日精选（最多 10 条，质量优先）+ 小结与展望（一段话）. Design-focused ~70%. Collection requires camofox-browser server (REST API). Run `npm run strict` for the full pipeline.
---
...
- **时效要求**：优先选择“今天新增信息”而非“今天再次讨论旧闻”；对评论型转述、旧闻复述、情绪化口吻内容默认降权。
- **事件优先级**：默认优先 `发布 / 上线 / 开源 / 内测 / 更新 / 评测`，`观点 / 吐槽 / 带货` 只作为补位，不应默认进入榜单。
- **补位约束**：`weak-signal`（如求助、担忧、访问异常、模糊感叹）默认不得入榜；`general` 只有在信息量足够、对象明确、动作明确时才允许补位。
- **源配置建议**：`references/query-presets.json` 除 handle 列表外，建议维护 `accountUrls` 映射；采集时优先使用显式 URL，减少大小写、改名与跳转带来的歧义。
...
- 今日精选条数最多 10 条；当日高质量候选不足时允许少于 10 条，禁止为凑数补低质量内容
- 每条摘要 100–140 字（按字符计），不足或超出均不通过
- 每条摘要不得以省略号（…、...、．）结尾，须完整成句
- 今日精选内不得出现重复 URL（同一 status 只允许出现一次）
- 小结与展望必须存在且为非空一段话
...
3. **用 AI 生成日报正文**：根据 `cache/candidates.json` 中的 10 条推文，由你（AI）撰写：
   - **今日精选**：每条一条**新闻式标题**（产品/事件+动作+一句话定位，禁止套用同一句式）+ **100–140 字中文摘要**（先写发生什么再写影响，设计师视角自然融入，禁止英文整句、禁止模板腔、禁止省略号结尾）+ 该条 `url` 用作链接。每条摘要严格 ≤140 字且完整成句，不得以「…」结尾；若超长请改写压缩而非截断。
   - **去重**：先通读 `cache/candidates.json` 全部候选，若发现多个条目属于**同一主题/同一产品/同一活动**（如飞书 Cli 发布，即使 URL 不同），只保留一条得分最高的，其余跳过，用候选池中其他主题补足 10 条。避免出现「TOP 10 中第 1 条与第 5 条都是某大会/某活动」这类同主题重复。
