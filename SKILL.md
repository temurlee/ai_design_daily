...
description: Generate a Chinese AI design daily report from real X/Twitter posts in the last 24 hours. Use when user asks for AI日报/设计日报/AI自媒体日报. Output: 列表（1～10 条，无「TOP 10」等标题）+ 小结与展望（一段话）. Design-focused ~70%. Prefer Camofox-based collection (cost-saving), with grok-search as optional fallback.
---

# AI设计日报 Skill

自动追踪指定关注账号的 X/Twitter 内容，生成设计相关日报并发送到 Teams。关注账号列表见 `references/query-presets.json`（不区分博主与官方）。
...
[产品/公司名 + 动作] — [一句话定位]
…
（共 N 条，1≤N≤10，无序号；直接展示条目，无 TOP 10/当日精选/📌 等标题）
...
- **范围**：内容范围为过去 24 小时；设计相关内容约占整体 70%；列表向设计倾斜。
...
- **必须完整遍历** `references/query-presets.json` 中关注账号列表（`bloggers` 与 `official` 合并后的全部账号）的每一个账号，不得跳过；某账号失败则记录后继续，最后汇报成功/失败列表。

### 2) 固定章节结构
必须包含且仅包含以下结构（列表区块无标题，不展示「TOP 10」「当日精选」等）：
- **列表**：1～10 条，每条为标题 + 摘要 + 链接；直接展示条目，不写 TOP 10、当日精选、📌 等任何标题或分隔行
- **🧭 小结与展望**（一段话）

### 2.5) 列表去重（禁止重复）
- **同一 URL/同一条推文**：只出现一次，禁止重复入榜。
- **同一主题或同一活动**：如同一场大会、同一产品同一波发布、同一条新闻的不同推文，**只保留一条最具代表性的**，其余从候选中用其他主题补足；避免同主题重复。

### 3) 单条内容格式（强制）
每条必须包含：
1. 产品/公司名 + 动作 + 一句话定位
2. 摘要**全部中文**（可保留必要术语一次括号英文），不得留英文整句或未翻译长段；严格 **100–140 字**，以新闻导语方式撰写：先写发生了什么事，再写影响或意义；禁止模板腔；**禁止以省略号（…、...、．）结尾**，须完整成句；不足 100 字须补写一句（如对设计/产品的意义），超过 140 字须改写压缩而非截断。
3. 设计师视角自然融入摘要
4. 可点击链接（`👉 [点击查看](URL)`）

### 红线清单（强制）
发送前脚本会逐项校验，**任一项命中即视为不合格，不发送并报错**：
- 列表条数须在 1～10 之间（候选几条就出几条，不凑、不截断）
- 每条摘要**须全部中文**，不得含未翻译英文整句或长段
- 每条摘要 **100–140 字**（按字符计），不足 100 或超过 140 均不通过
- 每条摘要不得以省略号（…、...、．）结尾，须完整成句
- 列表内不得出现重复 URL（同一 status 只允许出现一次）
- 小结与展望必须存在且为非空一段话
...
- [ ] 直接展示新闻条目，无列表标题或 📌；列表后为 🧭 小结与展望，带分割线
- [ ] 列表 1～10 条，每条为三行结构：标题行（加粗）+ 摘要行 + 链接行
...
- [ ] 列表内无同一条推文重复（同一 URL 只出现一次）；无同主题重复（如同一大会、同一活动只保留一条）
...
回归检查时，至少确认：
- 顶部三行顺序不变
- 列表直接展示条目，仅带 separator；小结与展望有标题且带 separator
- 列表每条严格三行结构（1～10 条）
- 链接均为 `👉 [点击查看](URL)`
- 小结与展望为单一段落
...
### 列表示例（1～10 条，此处仅列 3 条，无序号；不展示 TOP 10/当日精选）
...
2. **阅读规范**：打开本 SKILL.md 与「用户目标与原则」「不可退化规则」「文风与表达硬规则」。若运行环境中有你个人的偏好文档（如 prompt.md），可自行参考，不作为本 skill 的必读或组成部分。

3. **用 AI 生成日报正文**：根据 `candidates` 中的推文（1～10 条，候选几条就出几条，不凑满 10 条），由你（AI）撰写：
   - **列表**：每条一条**新闻式标题**（产品/事件+动作+一句话定位，禁止套用同一句式）+ **100–140 字中文摘要**（先写发生什么再写影响，设计师视角自然融入，禁止英文整句、禁止模板腔、禁止省略号结尾）+ 该条 `url` 用作链接。每条摘要严格 ≤140 字且完整成句，不得以「…」结尾；若超长请改写压缩而非截断。**直接展示新闻条目**，不写 TOP 10、当日精选、📌 等任何标题或分隔行。
   - **去重**：选条时**同一 URL 只出现一次**；若多条候选属于**同一主题/同一活动**，只保留一条最具代表性的，其余从候选中用其他主题补足。
   - **小结与展望**：**一段话**总结当天 AI 整体情绪与趋势、展望可能持续发酵的话题，可侧重设计/产品形态短期影响。
   - **生成后自检**：逐条检查——每条摘要 **100–140 字**、**不以省略号（…、...、．）结尾**、**无未翻译英文**；不足 100 字则补写一句，超 140 字或省略号结尾则改写压缩该条，有英文则改为中文表述。全部通过后再写入文件，减少被脚本红线打回重试。
